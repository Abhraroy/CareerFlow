...
interface JobCoverLetterTabProps {
  coverLetter: string
  copiedCoverLetter: boolean
  onCopyCoverLetter: () => void
}

/**
 * Tab Content: Tailored Cover Letter.
 * Formatted letter display with one-click copy button.
 */
export function JobCoverLetterTab({
  coverLetter,
  copiedCoverLetter,
  onCopyCoverLetter
}: JobCoverLetterTabProps): React.JSX.Element {
  return (
    <div className="flex flex-col gap-4">
      <pre className="bg-slate-50 dark:bg-[#18181b] border border-slate-200 dark:border-white/10 p-5 rounded-2xl text-xs text-slate-800 dark:text-neutral-200 font-sans leading-relaxed whitespace-pre-wrap shadow-xs">
        {coverLetter}
      </pre>
      <button
